...
def get_json_object(object):
    """
    get_json_object, used to convert a non json data to a json object
...
def delete_server(conn_param,server_param):
    """
    delete_server, used to delete the server, server.

    :param conn_param: the connection object, which represents the port to many elements of the api, in this case is compute.
    :param server_param: The value can be either the ID of a server or a Server instance.
    ...
    ignore_missing (bool) – When set to False ResourceNotFound will be raised when the server does not exist. 
    ...
    """
    conn_param.compute.delete_server(server_param, ignore_missing=True, force=False)
...
if __name__ == "__main__":
    ...
    * fill the stars with your data 
    ...
    print(get_server_field(compute,"central-minion-1","name"))
    """

    for server in compute_.servers():
        print(server.id)
